...
def stosorok(message):
    ...
    lyrics = {
        ...
        "а может быть зайдём": "На чай.",
        "на стрелки не гляди": "Пойдем...",
        "немного посидим": "Вдвоем.",
        "для нас с тобой урок": "Смешно...",
        "ты тоже одинок": "Давно...",
        "любовь не обмануть": "Ну, да...",
        "а если все вернуть": "Назад?"
    }
    ...
    # Проверяем, есть ли текст в словаре и отвечаем
    if text in lyrics:
        bot.send_message(message.chat.id, lyrics[text])

        # Если это "Рассвет..." или "Назад?", бот начинает отправлять припев
        if lyrics[text] in ["Рассвет...", "Назад?"]:
            time.sleep(2)  # Даем небольшую паузу перед припевом
            for line in chorus:
                bot.send_message(message.chat.id, line)
